#!/usr/bin/env bash
# Creates the widget style list and compiles all styles into CSS.
...
widget_files=$(find "$cwd/widget" -name "*.scss" -type f | LC_ALL=C sort)
...
printf '%s\n' '// Generated by style/compile/build.sh; do not edit.' > "$widgets_index_file"
...
if sass "$input_file" "$output_file" --style=expanded --quiet --no-source-map; then
