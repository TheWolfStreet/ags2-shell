...
function parsePayload(value: unknown): DropPayload | null {
	...
	const getFiles = Reflect.get(value, "get_files")
	const files = typeof getFiles === "function" ? getFiles.call(value) : null
	if (Array.isArray(files)) {
		const paths = Array.from(new Set(files.map(file => {
			if (!file || typeof file !== "object")
				return null
			const getPath = Reflect.get(file, "get_path")
			return typeof getPath === "function" ? getPath.call(file) : null
		}).filter((path): path is string => typeof path === "string" && path.length > 0)))
		if (paths.length > 0)
			return { paths, operation: "move" }
	}

	const getString = Reflect.get(value, "get_string")
	const duplicateString = Reflect.get(value, "dup_string")
	let text: unknown = null
	if (typeof getString === "function")
		text = getString.call(value)
	else if (typeof duplicateString === "function")
		text = duplicateString.call(value)
	if (typeof text === "string")
		return parsePayload(text)

	const getPath = Reflect.get(value, "get_path")
	const path = typeof getPath === "function" ? getPath.call(value) : null
	return typeof path === "string" && path.trim() ? { paths: [path.trim()], operation: "move" } : null
}
...
function preferredOperation(target: Gtk.DropTarget, operation: "copy" | "move") {
	if (operation === "copy")
		return "copy"
	const state = target.get_current_event_state?.() ?? 0
...
export function createDrag(grid: GridModel): DesktopDrag {
...
export function DragLayer({ drag }: { drag: DesktopDrag }) {
		...
		<Gtk.Fixed
			...
			$={fixed => {
				...
				const place = () => {
					...
					if (!session.enabled() || !ghost || !drag.hovered.peek()) {
