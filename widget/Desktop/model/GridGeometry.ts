...
export function findPathsIntersectingRectangle(
	...
	const normalized = reconcileGridPositions(fileList, positions, slotCount, grid.columns)
...
export function nearestSlotIndexForPoint(x: number, y: number, grid: GridMetrics): number {
...
export function movePathsToSlot(layout: SlotLayout, files: DesktopFile[], move: SlotMove): Record<string, number> {
	const normalized = reconcileGridPositions(files, layout.positions, layout.slotCount, layout.columns)
