...
async function readDdcBrightness(display: number): Promise<number | null> {
	...
	const match =
		...
		result.value.match(/\b10\s+(\d+)\s+\d+\b/)
...
async function setDdcBrightness(
...
class Brightness extends GObject.Object {
	...
	#displayWrite = debounce(DISPLAY_WRITE_DEBOUNCE_MS, () =>
		this.#flushDisplayWrites(),
	)
	#refreshExternal = debounce(EXTERNAL_REFRESH_DEBOUNCE_MS, () =>
		this.#refreshExternalDisplays(),
	)
	...
	async #loadKeyboardValue() {
		...
		this.#keyboardValue =
			this.#keyboardMax > 0 ? clamp01(current / this.#keyboardMax) : 0
	}
	...
	#watchDevices() {
		if (this.#displayDevice) {
			...
			this.#deviceMonitors.push(
				monitorFile(displayPath, (path) => {
	...
	#setKeyboardFromRaw(raw: number) {
		...
		if (next === this.#keyboardValue) return
	...
	#updateDisplayAvailable() {
		const next =
			this.#displayDevice.length > 0 || this.#externalDisplays.length > 0

		if (next === this.#displayAvailable) return
	...
	async #refreshExternalDisplays() {
		...
		if (next.length > 0) this.#initialized = true
		this.#updateDisplayAvailable()
	...
	async #flushDisplayWrites() {
		if (this.#displayWriteInFlight) return
		...
		if (target === this.#lastAppliedDisplayTarget) return
		...
		try {
			...
			if (writeSucceeded) {
				...
				console.error(
					`brightness.write: No display accepted brightness ${target}%`,
				)
				if (
					this.#queuedDisplayTarget === null &&
					this.#lastAppliedDisplayTarget !== null
				) {
	...
	async #applyDisplayBrightness(target: number): Promise<boolean> {
		...
		const externalWriteSucceeded = await setDdcBrightness(
			this.#externalDisplays,
			target,
		)
	...
	@setter(Number)
	set display(percent) {
		if (!this.#initialized && !this.#displayDevice) return
	...
	vfunc_finalize() {
		...
		for (const monitor of this.#deviceMonitors) monitor.cancel()
		this.#deviceMonitors = []
